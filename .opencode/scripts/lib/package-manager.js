 ...
 * 2. Project-specific config (in .agent/package-manager.json)
 * 3. package.json packageManager field
 * 4. Lock file detection
 * 5. Global user preference (in ~/.agent/package-manager.json)
...
function getPackageManager(options = {}) {
  ...
  const projectConfigPath = path.join(projectDir, '.agent', 'package-manager.json');
...
function setProjectPackageManager(pmName, projectDir = process.cwd()) {
  ...
  const configDir = path.join(projectDir, '.agent');
...
function getSelectionPrompt() {
  ...
  message += '  - Or add to ~/.agent/package-manager.json: {"packageManager": "pnpm"}\n';
