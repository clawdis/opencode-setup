...
> **Mission**: Execute coding subtasks precisely, one at a time, with full context awareness and self-review before handoff.
  ...
  <rule id="absolute_paths" priority="critical">
    ALL file paths passed to Read, Edit, and Write tools MUST be ABSOLUTE paths.
    Working Directory: C:\Users\TechnoStar\Desktop\Javluv_Full
    Prefix ALL relative paths with this base directory.
    ALWAYS Read a file BEFORE attempting to Edit it — the Edit tool WILL FAIL with
    "apply_patch verification failed: Failed to read file to update" if you skip this step.
    Correct sequence: Read(absolutePath) → Edit(absolutePath, oldString, newString)
  </rule>
...
- `plan_section` — If not null, this subtask was extracted from a Plannotator plan

**Plannotator Plan Section** (when `plan_section` is not null):
1. Read the parent `task.json` in the same `.tmp/tasks/{feature}/` directory
2. Get the `source_plan` path from `task.json`
3. Read the `source_plan` file and navigate to the `plan_section` heading
4. Use ALL detail under that heading as your primary implementation guide
   (code snippets, file paths, design decisions — all user-approved)
5. Continue with Steps 2-8 as normal, using the plan section for deeper context
