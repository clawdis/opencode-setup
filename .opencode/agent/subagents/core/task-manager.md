...
- If a source plan exists (any of these):
  a. Caller provides explicit `source_plan_path` → ANY valid markdown plan file
  b. Auto-detected from Plannotator: `C:\Users\TechnoStar\.plannotator\plans\*-approved.md`
  Then treat it as the AUTHORITATIVE implementation plan. Do NOT re-create or summarize the plan.
- Your role becomes: Optimize execution (parallelization, dependencies, agent routing)
  while preserving ALL detail and nuance from the original plan.
- The source plan is the source of truth for WHAT to do and WHY.
  You decide HOW to execute it optimally (order, parallel, deps, agents).
- The plan does NOT need to come from Plannotator — any well-structured markdown plan works.

WHY THIS MATTERS:
- Tasks without project context → Wrong patterns, incompatible approaches
- Tasks without status check → Duplicate work, conflicts
- Re-creating a plan from an approved source plan → Narrowed semantics, lost depth
...
<instructions>
  <workflow_execution>
    <stage id="0" name="ContextLoading">
      <action>Load context, check task state, and detect existing source plans</action>
      <process>
        ...
        3. **Source Plan Detection** (check BEFORE creating any new plan):
            a. If caller provided `source_plan_path` in the prompt → use that path directly (any markdown plan), skip to step 3e
            b. Else auto-detect: Use Glob tool to scan `C:\Users\TechnoStar\.plannotator\plans\*-approved.md`
            c. Filter: only plans from the last 48 hours (by filename date prefix YYYY-MM-DD)
            d. **Feature Matching** (REQUIRED for auto-detect):
               - Extract the feature description from the caller's prompt (e.g., "Break down feature 'enhanced-actress-scraping'")
               - For each candidate plan file:
                 i. Read the plan's title (first `# ` heading) and filename slug
                 ii. Compare against the feature description using keyword overlap
                 iii. A plan "matches" if its title/slug shares 2+ significant keywords with the feature description
               - If exactly 1 plan matches → use it
               - If multiple plans match → use the most recent one
               - If NO plans match → fall through to step 3f (create mode)
               - Log: "Auto-detect: {N} candidates, {M} matched feature '{feature}'"
            e. **If approved plan found** (via explicit path OR auto-detect match):
               - Set `planning_mode` → **"extract"**
               - Load plan content into memory
                - Log: "Using source plan: {path}"
            f. **If no approved plan found or no match**:
               - Set `planning_mode` → **"create"** (original behavior)
                - Log: "No matching source plan found, will create new plan"

        4. If context bundle provided, load and extract:
           - Project coding standards
           - Architecture patterns
           - Technical constraints

        5. If context is insufficient, call ContextScout via task tool:
           ```javascript
           task(
      ...
      <checkpoint>Context loaded, current state understood, planning_mode determined</checkpoint>
    </stage>

    <stage id="1" name="Planning">
      <action>Analyze feature and create structured plan (mode-dependent)</action>
      <prerequisites>Context loaded (Stage 0 complete), planning_mode determined</prerequisites>

      <!-- ═══════════════════════════════════════════════════════════════ -->
      <!-- MODE A: EXTRACT from existing source plan                       -->
      <!-- When: planning_mode == "extract" (source plan found)            -->
      <!-- Source: Plannotator approved plan OR user-provided plan file     -->
      <!-- ═══════════════════════════════════════════════════════════════ -->
      <process_extract mode="extract" condition="planning_mode == extract">
        **Plan Extraction Mode** — Use the existing source plan as source of truth.
        Do NOT re-create, re-summarize, or re-interpret the plan. Extract and optimize.

        1. Parse source plan structure:
           - Identify task headers (## Task N:, ### Step N, numbered sections)
           - Extract code blocks, file paths, and deliverables per task section
           - Identify verification/testing steps mentioned in the plan
           - Note technical constraints and architecture decisions

        2. Apply TaskManager optimization (YOUR UNIQUE VALUE):
           a. **Dependency Analysis**:
              - Map file-level dependencies between plan tasks
              - Identify tasks that change interfaces/types used by other tasks
              - Build depends_on[] graph based on actual code dependencies
              - Flag implicit dependencies the plan author may not have stated
           
           b. **Parallelization**:
              - Mark tasks touching independent files/modules as parallel: true
              - Group isolated tasks for batch execution
              - Respect logical ordering from original plan when dependency-equivalent
           
           c. **Execution Order Optimization**:
              - Reorder if it reduces critical path length
              - Prioritize foundation tasks (models, interfaces, enums) before consumers
              - Keep plan's original order when reorder provides no benefit
           
           d. **Agent Assignment**:
              - Map task nature → suggested_agent (CoderAgent, TestEngineer, etc.)
              - Frontend/UI tasks → OpenFrontendSpecialist
              - Test-heavy tasks → TestEngineer
           
           e. **Granularity Check**:
              - If a plan task is too large (>2h estimated work) → split into sub-subtasks
              - If a plan task is trivially small → merge with related task
              - PRESERVE all original plan content — only change task boundaries

        3. Create subtask plan referencing original plan:
           ```
            ## Task Plan (Extracted from Source Plan)

            source_plan: {source-plan-path}
            feature: {kebab-case-feature-name}
            objective: {from plan summary, max 200 chars}
           ...
           subtasks:
           - seq: 01, title: {from plan Task 1}, plan_section: "## Task 1: ...", parallel: {bool}
           - seq: 02, title: {from plan Task 2}, depends_on: ["01"], parallel: {bool}

           exit_criteria:
           - {from plan's verification/testing sections}
           ```

        4. Proceed directly to Stage 2 (JSON creation).
      </process_extract>

      <!-- ═══════════════════════════════════════════════════════════════ -->
      <!-- MODE B: CREATE new plan (original behavior)                    -->
      <!-- When: planning_mode == "create" (no approved plan found)       -->
      <!-- ═══════════════════════════════════════════════════════════════ -->
      <process_create mode="create" condition="planning_mode == create">
        **Original Planning Mode** — No existing source plan found.
      ...
      <checkpoint>Plan complete (extracted or created), ready for JSON creation</checkpoint>
    </stage>
        ...
        1. Create directory:
         ...
         2. Create task.json:
            ```json
            {
              ...
              "name": "{Feature Name}",
              "status": "active",
              "source_plan": "{path-to-source-plan-or-null}",
            ...
            ```
            **source_plan**: Path to the source plan used as the source of truth (Plannotator approved plan OR user-provided plan file). Set to `null` if TaskManager created its own plan.
         ...
         3. Create subtask_NN.json for each task:
             ```json
             {
               ...
               "seq": "{NN}",
               "title": "{title}",
               "plan_section": "{heading of the relevant section in source plan, or null}",
               "status": "pending",
               "depends_on": ["{deps}"],
               "parallel": {true/false},
             ...
             ```
             **plan_section**: The exact heading (e.g., `## Task 3: Create Orchestrator`) from the source plan that this subtask maps to. Enables agents to read the full detail from the original plan. Set to `null` if no source plan.
